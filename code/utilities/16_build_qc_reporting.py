...
def build_sample_mask(
    ...
    # I rebuild the censor mask here from the confounds so the QC report uses the same logic as denoising.
    ...
    expanded_spike_mask = expand_spike_mask(spike_mask, fd_backward_neighbors, fd_forward_neighbors)
    keep_mask &= ~expanded_spike_mask
    return keep_mask, expanded_spike_mask, dvars_column


def draw_flow_box(ax: plt.Axes, x: float, y: float, width: float, height: float, title: str, text: str, color: str) -> None:
...
def plot_sample_flow(screening_dir: Path, outpath: Path) -> None:
    # This is the quick visual version of how the sample got smaller at each step.
    mri = pd.read_csv(screening_dir / "ds005752_mri_participants.tsv", sep="\t")
    young_remote = pd.read_csv(screening_dir / "ds005752_mri_participants_age_20_25_remote_anat_forward.tsv", sep="\t")
    older_remote = pd.read_csv(screening_dir / "ds005752_mri_participants_age_50_75_remote_anat_forward.tsv", sep="\t")
    qc_pass = pd.read_csv(screening_dir / "ds005752_qc_pass_sample.tsv", sep="\t")
    final_sample = pd.read_csv(screening_dir / "ds005752_final_analysis_sample_tr_3s.tsv", sep="\t")
    ...
    steps = [
        ("MRI-eligible in age band", f"{counts['young_age_band']} young\n{counts['older_age_band']} older"),
        ("Remote anat + forward-rest candidates", f"{counts['young_remote']} young\n{counts['older_remote']} older"),
...
def plot_within_mean_stability(network_df: pd.DataFrame, outpath: Path) -> None:
    ...
    data = [network_df.loc[network_df["network"] == network, "within_mean_z"].to_numpy() for network in networks]
    box = ax.boxplot(
        data,
        labels=networks,
        patch_artist=True,
...
def get_confounds_path(derivatives_dir: Path, subject_id: str) -> Path:
    ...
    return next(func_dir.glob(f"{subject_id}_ses-01_task-rest_dir-forward_desc-confounds_timeseries.tsv"))
...
def plot_motion_trace(
    ...
    fd = pd.to_numeric(confounds["framewise_displacement"], errors="coerce") if "framewise_displacement" in confounds.columns else pd.Series(np.nan, index=confounds.index)
    dvars = pd.to_numeric(confounds[dvars_column], errors="coerce") if dvars_column else pd.Series(np.nan, index=confounds.index)
    x = np.arange(len(confounds))

    fig, axes = plt.subplots(3, 1, figsize=(10.8, 6.8), sharex=True, gridspec_kw={"height_ratios": [1, 1, 0.45]})

    axes[0].plot(x, fd, color=YOUNG_COLOR, linewidth=1.25)
    axes[0].axhline(fd_threshold, color=REFERENCE_LINE_COLOR, linestyle="--", linewidth=1.4)
    style_axis(axes[0], title=f"{subject_id}: FD, DVARS, and censor mask", ylabel="FD (mm)")
    axes[0].grid(color=GRID_COLOR, alpha=0.22, linewidth=0.8)

    axes[1].plot(x, dvars, color=OLDER_COLOR, linewidth=1.25)
    if dvars_threshold is not None and dvars_column:
        axes[1].axhline(dvars_threshold, color=REFERENCE_LINE_COLOR, linestyle="--", linewidth=1.4)
    ...
    if len(censored_idx):
        axes[2].scatter(censored_idx, np.zeros_like(censored_idx), color=REFERENCE_LINE_COLOR, s=6, alpha=0.8)
...
def build_motion_qc_figures(
    ...
    dvars_threshold = (
        None if pd.isna(denoising_settings.get("dvars_threshold", np.nan))
    ...
    for subject_id in sorted(sample["subject_id"].tolist()):
        # I make one compact motion figure per subject so the later QC check stays manageable.
        confounds = pd.read_csv(get_confounds_path(derivatives_dir, subject_id), sep="\t")
...
def compute_qcfc(matrices_dir: Path, denoising_summary: pd.DataFrame, atlas_data_dir: Path, n_rois: int) -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    # QC-FC is the main check for whether motion is still leaking into connectivity after denoising.
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        r_vals = np.divide(fd_centered @ edges_centered, denom, out=np.zeros_like(denom), where=denom > 0)
...
def plot_qcfc_distribution(qcfc_df: pd.DataFrame, outpath: Path) -> None:
    ...
    style_axis(ax, title="QC-FC Edge Distribution", xlabel="Edgewise correlation with mean FD", ylabel="Number of edges")
    ax.grid(axis="y", color=GRID_COLOR, alpha=0.28, linewidth=0.9)
    save_figure(fig, outpath)
...
def plot_qcfc_distance(qcfc_df: pd.DataFrame, outpath: Path) -> None:
    ...
    qcfc_df["distance_bin"] = pd.cut(qcfc_df["distance_mm"], bins=bins, include_lowest=True)
    binned = qcfc_df.groupby("distance_bin", observed=True).agg(
        distance_mid=("distance_mm", "mean"),
        qcfc_mean=("qcfc_r", "mean"),
    )
    ax.plot(binned["distance_mid"], binned["qcfc_mean"], color=OLDER_COLOR, linewidth=2.0)
    ...
    style_axis(ax, title="QC-FC by Inter-Parcel Distance", xlabel="Distance between parcel centroids (mm)", ylabel="QC-FC correlation (r)")
...
def main() -> None:
    ...
    denoising_summary = denoising_summary[denoising_summary["subject_id"].isin(subject_ids)].copy()
    ...
    plot_retained_minutes(denoising_summary, figures_dir / "retained_minutes_distribution.png")
    ...
    motion_summary.to_csv(output_dir / "motion_qc_trace_summary.tsv", sep="\t", index=False)
        ...
        [
            {
                ...
                "within_mean_z_n_le_zero": int((network_df["within_mean_z"] <= 0).sum()),
                "within_mean_z_n_abs_lt_0p05": int((network_df["within_mean_z"].abs() < 0.05).sum()),
            }
        ]
    )
